...
Start with the comprehensive [Commercials Guide](./COMMERCIALS_GUIDE.md). The focused references are [Commissioning](./COMMISSIONING.md), [Pricing Baseline and Maintenance](./PRICING_BASELINE_AND_MAINTENANCE.md), [Commission Payout Runbook](./COMMISSION_PAYOUT_RUNBOOK.md), [Operating Model](./BANK_OPERATING_MODEL.md), [Offering Runbook](./OFFERING_RUNBOOK.md), [Settlement Operations](./SETTLEMENT_OPERATIONS.md), and [Architecture](./ARCHITECTURE.md). The cross-domain Maker/Checker policy is documented in [Commissioning Maker/Checker Doctrine](../governance/COMMISSIONING_MAKER_CHECKER_DOCTRINE.md).
