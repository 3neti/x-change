...
const canonicalInstitutions = computed<MoneyIssuerOption[]>(() => {
  ...
  return availableBanks.value.map((bank) => ({
    ...
    category: getPopularEMIs().some((emi) => emi.code === bank.code)
      ? "wallet"
      : "bank",
...
const commonInstitutions = computed(() =>
  matchingInstitutions.value.filter((institution) => institution.commonly_used),
);

const otherInstitutions = computed(() =>
  matchingInstitutions.value.filter(
    (institution) => !institution.commonly_used,
  ),
);
