...
const distributionLinksAvailable = computed(
  () =>
    beneficiaryRedeemUrl.value !== null || beneficiaryRedeemPath.value !== null,
);
const amountDisplay = computed(() =>
  moneyValue(summary.value.amount, stringValue(summary.value.currency)),
);
const claimStateDisplay = computed(() =>
  claimState(summary.value.claimed, summary.value.fully_claimed),
);
const redemption = computed<Record<string, unknown> | null>(() =>
  objectValue(summary.value.redemption),
);
const redemptionStatus = computed(() =>
  settlementStatusLabel(stringValue(redemption.value?.status)),
);
...
const redemptionProvider = computed(() =>
  providerLabel(stringValue(redemption.value?.provider)),
);
const redemptionRail = computed(() =>
  settlementRailLabel(stringValue(redemption.value?.settlement_rail)),
);
...
const redemptionReference = computed(
  () => stringValue(redemption.value?.provider_transaction_id) ?? "Pending",
);
const availabilityDisplay = computed(() =>
  availabilityWindow(summary.value.starts_at, summary.value.expires_at),
);
...
const primaryNextStep = computed(() => {
  if (distributionLinksAvailable.value) {
    return {
      ...
      description:
        "Manual distribution is available from this read-only detail screen.",
    };
  }
  ...
  return {
    ...
    description:
      "Distribution links are not available from the current read model.",
  };
});
const lifecycleGuidance = computed(() => {
  ...
  if (normalizedStatus.includes("payout_rejected")) {
    return {
      ...
      message:
        "The claim is recorded, but the provider rejected the payout. The protected principal requires destination correction.",
    };
  }
  ...
  if (normalizedStatus.includes("payout_pending")) {
    return {
      ...
      message:
        "The claim is recorded while provider settlement remains in progress.",
    };
  }
  ...
  if (normalizedStatus === "paid") {
    return {
      ...
      label: "Paid",
      message:
        "The provider payout succeeded. Expiry no longer changes this completed financial outcome.",
    };
  }
  ...
  if (normalizedStatus.includes("expired")) {
    return {
      ...
      label: "Expired",
      message:
        "The Pay Code appears expired. Review evidence before manual distribution.",
    };
  }

  if (
    normalizedStatus.includes("redeemed") ||
    normalizedStatus.includes("claimed")
  ) {
    return {
      tone: "complete",
      label: "Claimed",
      message:
        "The Pay Code appears claimed. Use this screen for read-only inspection.",
    };
  }
  ...
  return {
    ...
    label: "Available",
    message:
      "The Pay Code is presented as available from the sanitized lifecycle summary.",
  };
});
  ...
  return [
    ...
    },
    {
      ...
      value: moneyValue(
        ...
        stringValue(summary.value.currency),
      ),
    ...
    },
    {
      ...
      helper:
        "Claim booleans are summary facts; claim payloads remain redacted.",
    },
    {
      key: "availability-window",
      label: "Availability window",
      value: availabilityWindow(
        summary.value.starts_at,
        summary.value.expires_at,
      ),
...
const timelineItems = computed<CockpitVoucherTimelineItem[]>(() => [
  ...
  },
  {
    ...
    label: "Availability",
    description:
      "Voucher starts/expires timestamps are rendered without claim or instruction payloads.",
    timestamp: availabilityWindow(
      summary.value.starts_at,
      summary.value.expires_at,
    ),
    source: "system",
  ...
  },
  {
    ...
    description:
      "Notification summaries remain unavailable until an authorized feedback read model is connected.",
...
const auditItems = computed<CockpitVoucherAuditItem[]>(() => [
  {
    ...
    helper:
      "Execution results remain not wired; Cockpit does not invoke drivers.",
  },
  ...journalAuditItems(props.read_model?.journal),
  {
    ...
    helper:
      "Follow-up guidance can be displayed here, but this page does not execute actions.",
  },
  {
    id: "feedback",
    label: "Notification status",
    ...
    helper:
      "Notification delivery remains unavailable until an authorized feedback read model is connected.",
  },
]);
...
const detailActions = computed<CockpitVoucherDetailAction[]>(() => {
  ...
  if (
    readModelStatus(props.read_model?.actions) !== "available" ||
    actions.length === 0
  ) {
  ...
  return actions
    .map((action, index) =>
      actionDetailItem(action, index, props.read_model?.actions?.redactions),
    )
...
const primaryEvidenceReadiness = computed(() =>
    ...
    return [
      {
        ...
        status: "not_wired",
        helper:
          "Claim evidence and uploaded artifacts are not exposed by the voucher summary read model.",
      },
      {
        ...
        status: "not_wired",
        helper:
          "Approval references and OTP metadata remain redacted from Cockpit voucher hydration.",
      },
      {
        ...
        status: "not_wired",
        helper:
          "Settlement Envelope readiness requires a future authorized read model.",
      },
    ];
  }
...
function journalAuditItems(
  model?: CockpitDependentReadModel,
): CockpitVoucherAuditItem[] {
    ...
    return [
      {
        ...
        helper:
          "Audit entries remain unavailable until an authorized journal read model is connected.",
      },
    ];
  }
...
function actionDetailItem(
  ...
  const key =
    stringValue(actionItem.key) ??
    stringValue(actionItem.id) ??
    `action-${index + 1}`;
  const label =
    stringValue(actionItem.label) ?? stringValue(actionItem.name) ?? key;
  ...
  const payloadPolicy =
    stringValue(actionRedactions?.payloads) ?? "safe-action-host-summary-only";
  const reasonParts = [
    description,
...
function feedbackDistributionItems(
  model?: CockpitDependentReadModel,
): CockpitVoucherDistributionItem[] {
  ...
  return deliveries
    .map((delivery, index) =>
      feedbackDistributionItem(delivery, index, model?.redactions),
    )
...
function feedbackDistributionItem(
  ...
  return {
    id:
      stringValue(deliveryItem.delivery_id) ??
      stringValue(deliveryItem.id) ??
      `feedback-${index + 1}`,
    channel: channelLabel(channel),
    status,
...
function integrationSummary(
  ...
  return {
    ...
    policy: policyLabel(
      stringValue(model?.redactions?.payloads) ?? "not-loaded",
    ),
    reason: policyLabel(
      stringValue(model?.redactions?.reason) ?? "read-model-ready",
    ),
  };
}
  ...
  >
    <section class="space-y-6" data-testid="cockpit-voucher-detail-shell">
      ...
      <div
        class="hidden"
        aria-hidden="true"
        data-testid="cockpit-voucher-detail-legacy-projection"
      >
        <section
        ...
        >
          ...
          >
            <div>
              ...
              <p
                class="mt-1 max-w-3xl text-sm leading-5 text-slate-600 dark:text-slate-300"
              >
                This summary is built from sanitized voucher read-model facts.
                It gives operators the current lifecycle state, beneficiary URL
                readiness, and safe next steps without mutating the Pay Code or
                triggering delivery.
              </p>
            </div>
            <span
              class="w-fit rounded-full bg-white px-3 py-1 text-xs font-semibold text-emerald-700 ring-1 ring-emerald-200 dark:bg-slate-950 dark:text-emerald-200 dark:ring-emerald-800"
            >
              read-only
            </span>
          </div>
          ...
          >
            ...
            >
              ...
              <p
                class="mt-0.5 text-[0.7rem] leading-4 text-slate-500 dark:text-slate-400"
              >
            ...
            >
              ...
              >
                {{
                  distributionLinksAvailable
                    ? "Manual copy/inspection only."
                    : "Waiting for distribution link read model."
                }}
              </p>
            </div>
          </dl>
          ...
          >
            <div class="flex flex-wrap items-center justify-between gap-2">
              ...
              <span
                class="rounded-full bg-emerald-100 px-2.5 py-1 text-xs font-semibold text-emerald-800 dark:bg-emerald-900/60 dark:text-emerald-200"
              >
                {{ redemptionStatus }}
              </span>
            </div>
          ...
          <div
            class="mt-4 grid gap-3 xl:grid-cols-[minmax(0,1.2fr)_minmax(20rem,0.8fr)]"
          >
            <div
            ...
            >
              ...
              <p
                class="mt-1.5 text-sm font-semibold text-slate-950 dark:text-slate-50"
              >
              ...
              <p
                class="mt-1.5 text-xs leading-5 text-slate-500 dark:text-slate-400"
              >
                Availability: {{ availabilityDisplay }} · Payload policy:
                {{ redactions.payloads ?? "not-loaded" }}
              </p>
              <div
            ...
            >
              <summary class="cursor-pointer list-none">
                ...
                <span
                  class="mt-1.5 flex flex-wrap items-center justify-between gap-2"
                >
                  <span
                  ...
                  <span
                    class="rounded-full bg-white px-2.5 py-0.5 text-[0.7rem] font-semibold text-slate-700 ring-1 ring-slate-200 dark:bg-slate-950 dark:text-slate-200 dark:ring-slate-800"
                  >
              ...
              <p
                class="mt-2 border-t border-current/10 pt-2 text-xs leading-5 text-slate-600 dark:text-slate-300"
              >
                {{ lifecycleGuidance.message }}
              </p>
              <p
                class="mt-1 text-[0.7rem] leading-4 text-slate-500 dark:text-slate-400"
              >
                Derived from display status only; Cockpit does not enforce
                lifecycle policy from this page.
              </p>
            </details>
          </div>
          ...
          >
            <summary
              class="flex cursor-pointer list-none flex-wrap items-center justify-between gap-2 text-xs font-semibold uppercase tracking-[0.18em] text-emerald-700 dark:text-emerald-300"
            >
              <span class="flex flex-wrap items-center gap-2">
                <span>Connected context</span>
                <span
                  class="rounded-full bg-white/80 px-2 py-0.5 text-[0.65rem] tracking-normal text-emerald-700 normal-case dark:bg-slate-950/70 dark:text-emerald-200"
                >
                  4 read-only facts
                </span>
              </span>
              <span
                class="rounded-full bg-emerald-100 px-2 py-0.5 text-[0.65rem] tracking-normal text-emerald-700 normal-case dark:bg-emerald-950 dark:text-emerald-200"
              >
                read-only
              </span>
            </summary>
            ...
            >
              Main inspection facts for claim access, notification state,
              follow-up guidance, and audit evidence.
            </p>
            <dl class="mt-2 grid gap-1.5 text-xs sm:grid-cols-2 xl:grid-cols-4">
              ...
              >
                <dt class="flex items-center justify-between gap-2">
                  <span
                    class="text-xs font-semibold text-slate-950 dark:text-slate-50"
                  >
                    {{ item.label }}
                  </span>
                  <span
                ...
                <dd
                  class="mt-1 text-xs font-semibold text-slate-950 dark:text-slate-50"
                >
                  {{ item.count }}
                </dd>
                <p
          ...
          >
            ...
            <p
              class="mt-2 text-xs leading-5 text-slate-600 dark:text-slate-300"
            >
              Read-only audit, follow-up, and notification state. These facts do
              not execute actions, send notifications, or write journal entries.
            </p>
            <dl class="mt-2 grid gap-1.5 text-xs md:grid-cols-3">
              ...
              >
                ...
                <dd
                  class="mt-1 text-[0.7rem] leading-4 text-slate-500 dark:text-slate-400"
                >
                  <span>{{ item.count }} · {{ item.policy }}</span>
                  <span
                    class="block"
                    data-testid="cockpit-voucher-detail-primary-evidence-readiness-reason"
                  >
                    {{ item.reason }}
                  </span>
                </dd>
        ...
        >
          ...
          >
            ...
            preserved. These links only change the read-only Cockpit view; they
            do not update campaign state, issue Pay Codes, send feedback, write
        ...
        >
          <summary class="cursor-pointer list-none">
            <div class="flex flex-wrap items-center justify-between gap-2">
              ...
              <span
                class="rounded-full bg-white/80 px-2 py-0.5 text-[0.65rem] font-semibold text-emerald-700 dark:bg-slate-950/70 dark:text-emerald-200"
              >
                URL details
              </span>
            </div>
            ...
            >
              Inspect the canonical claim URL, source metadata, and manual
          ...
          >
            <p class="text-xs leading-5 text-slate-600 dark:text-slate-300">
              Copying is browser-local; delivery disabled means recipient
              verification and delivery remain external.
            </p>
          </div>
          <div
          ...
          <dl class="mt-3 grid gap-2 text-xs">
            <div class="rounded-lg bg-white/80 p-2.5 dark:bg-slate-950/70">
              ...
              <dd
                class="mt-1 break-all font-semibold text-slate-950 dark:text-slate-50"
              >
                <a
                  v-if="beneficiaryRedeemUrl"
                  :href="beneficiaryRedeemUrl"
          ...
          >
            <summary
              class="cursor-pointer font-semibold text-slate-950 dark:text-slate-50"
            >
            ...
            <ul class="mt-2 list-disc space-y-1 pl-5">
              ...
              <li>
                Share it only through an approved external workflow after
                verifying the recipient.
              </li>
              <li>
                Cockpit does not send SMS, email, webhook, in-app notification,
                or campaign delivery from this panel.
              </li>
              <li>
                Cockpit does not record copy telemetry, create short links, or
                generate QR assets here.
              </li>
              <li>
                Treat this beneficiary URL as sensitive settlement access
                material.
              </li>
            </ul>
          </details>
        ...
        <div class="space-y-3" data-testid="cockpit-voucher-secondary-content">
          ...
          >
            ...
            <div
              class="space-y-3"
              data-testid="cockpit-voucher-supporting-evidence-stack"
            >
