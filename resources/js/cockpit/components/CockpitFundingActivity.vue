...
function statusTone(status: CockpitFundingActivityItem["status"]): string {
  return {
    ...
    cancelled:
      "bg-slate-200 text-slate-700 dark:bg-slate-800 dark:text-slate-200",
    reversed: "bg-rose-100 text-rose-800 dark:bg-rose-950 dark:text-rose-200",
  }[status];
}
      ...
      <div class="hidden overflow-x-auto md:block">
        <table class="w-full min-w-[48rem] text-left text-sm">
          <caption class="sr-only">
            Account Funding requests, observations, and recognition status
          </caption>
          <thead
          ...
          <tbody>
            ...
            >
              ...
              <td class="px-4 py-3">
                <code class="text-xs font-semibold tracking-wide">
                  {{ item.display_reference }}
                </code>
                <p
                  v-if="item.summary && item.summary !== item.display_reference"
      ...
      >
        ...
        >
          ...
          >
            ...
            <span
              class="max-w-28 rounded-full px-2.5 py-1 text-center text-[0.625rem] leading-4 font-semibold break-words whitespace-normal uppercase"
              :class="statusTone(item.status)"
          ...
          <div
            class="flex min-w-0 flex-col items-start gap-3 min-[360px]:flex-row min-[360px]:items-end min-[360px]:justify-between"
          >
            <div class="min-w-0">
            ...
            <div
              class="flex min-w-0 flex-wrap justify-start gap-2 min-[360px]:justify-end"
            >
              <button
                v-for="action in item.action_keys"
                :key="action"
