...
describe('Cockpit funding QR address', () => {
    it('centers a prominent QR beneath its authoritative payer label', () => {
        ...
        expect(
            wrapper
                .get('[data-testid="funding-qr-merchant-label-preview"]')
                .text(),
        ).toBe('TREASURY DESK - MANILA');
        expect(
            wrapper
                .get('[data-testid="funding-qr-merchant-label-state"]')
                .text(),
    ...
    it('previews format changes and blocks labels over 25 characters', async () => {
        ...
        expect(
            wrapper
                .get('[data-testid="funding-qr-merchant-label-count"]')
                .text(),
        ).toContain('8 / 25');
        ...
        expect(
            wrapper
                .get('[data-testid="funding-qr-update"]')
                .attributes('disabled'),
        ).toBeDefined();
    });
