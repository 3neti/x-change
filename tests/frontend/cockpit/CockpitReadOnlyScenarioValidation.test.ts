...
const voucherReadModel = {
  ...
  voucher: {
    ...
    overview: {
      ...
      timing: {
        issued_at: "2026-07-09T10:00:00+08:00",
        starts_at: "2026-07-09T10:00:00+08:00",
        expires_at: "2026-07-16T10:00:00+08:00",
        redeemed_at: null,
      },
...
describe("Cockpit read-only UI/UX scenario validation checkpoint", () => {
  ...
  it("renders a scenario voucher detail as visible read-only evidence and communication summary", async () => {
    ...
    expect(wrapper.text()).toContain("Value is safely held and ready");
    expect(
      wrapper
    ...
    const audit = wrapper.get('[data-testid="pay-code-audit-tab"]');

    expect(audit.text()).toContain("Journal");
    expect(audit.text()).toContain("Voucher Generated");
    expect(audit.text()).toContain("Voucher Slice Redeemed");
    expect(audit.text()).toContain("Delivery");
    expect(audit.text()).toContain("Sms");
    expect(audit.text()).toContain("Planned");
    expectNoUnsafeText(wrapper.text());
  });
});
