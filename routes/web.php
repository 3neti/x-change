...
use LBHurtado\XChange\Http\Controllers\Web\Cockpit\CockpitPayCodeCollectionAttemptController;
use LBHurtado\XChange\Http\Controllers\Web\Cockpit\CockpitPayCodeEngineeringPreviewController;
...
Route::prefix('x')->middleware([...$middleware, ShareXChangeBranding::class])->group(function (): void {
    ...
    Route::prefix('cockpit')->middleware(ShareCockpitHeaderReadModel::class)->group(function (): void {
        ...
        Route::post('campaigns/{worksheet}/fulfillments/fallbacks', [CockpitCampaignWorksheetFallbackController::class, 'store'])
            ->middleware('throttle:6,1')
            ->name('x-change.cockpit.campaigns.fulfillments.fallbacks.store');
        ...
        Route::prefix('pay-codes')->group(function (): void {
            ...
            Route::get(
                '{code}/engineering-preview',
                CockpitPayCodeEngineeringPreviewController::class,
            )->middleware('throttle:30,1')
                ->name('x-change.cockpit.pay-codes.engineering-preview.show');
            Route::post(
                '{code}/collection-attempts',
